...
export function personalize(state, choices) {
  ...
  if(![2,3,4,5].includes(choices.trainingTarget) || typeof choices.rower!=='boolean' || typeof choices.walker!=='boolean' || typeof choices.rope!=='boolean' || !Number.isInteger(choices.readingMinutes) || choices.readingMinutes<1 || choices.readingMinutes>30 || !Number.isInteger(choices.readingTarget) || choices.readingTarget<1 || choices.readingTarget>7 || typeof choices.adaptive!=='boolean')throw Error('Vérifie les objectifs renseignés.');
  ...
  next.modules.training.data.walker=choices.walker;
  next.modules.training.data.rope=choices.rope;
  next.modules.training.data.walkerMaxSpeed=15;
  next.modules.training.data.walkerMaxIncline=15;
  if(choices.walker && !next.modules.training.data.scores.walk)next.modules.training.data.scores.walk={a:5,s:0};
  if(choices.rope && !next.modules.training.data.scores.rope)next.modules.training.data.scores.rope={a:20,s:0};
...
export function mountProfile({root,state,save,restore}) {
  ...
  root.querySelector('#profile-form').onsubmit=e=>{
    ...
    try{save(personalize(state,{name:form.get('name'),modules:form.getAll('modules'),trainingTarget:Number(form.get('trainingTarget')),rower:form.has('rower'),walker:form.has('walker'),rope:form.has('rope'),readingMinutes:Number(form.get('readingMinutes')),readingTarget:Number(form.get('readingTarget')),adaptive:form.has('adaptive')}));}
